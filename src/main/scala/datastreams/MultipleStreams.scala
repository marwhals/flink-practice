...
object MultipleStreams {
  ...
  // Unioning = combining the output of multiple streams into one
  def demoUnion(): Unit = {
  ...
  // window join - two streams of potentially two different types in the same window + some join condition
  ...
  // interval joins = correlation between events A and B if duration Min < timeA - timeB < durationMax. This involves event time and only works on Keyed Streams
  ...
  def demoConnect(): Unit = {
    ...
    // Envinronmet variables - will use single-threaded
